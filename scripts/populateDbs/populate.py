...
def generate_posts(users: List[dict], posts_per_user: int, created_at: datetime.datetime, target_user_id: str = None) -> List[dict]:
        ...
        if target_user:
            for _ in range(posts_per_user):
                posts.append({
                    ...
                    "content": f"Autogenerated post by {target_user['name']}",
                    "created_at": created_at,
                    "user_id": target_user["id"],
                    "likes": 0,
                })
        else:
            print(f"Warning: Target user {target_user_id} not found in users list")
    else:
...
def generate_follows(users: List[dict], follows_count: int, start: datetime.datetime, end: datetime.datetime, target_user_id: str = None) -> List[dict]:
...
def to_sql(users: List[dict], posts: List[dict], likes: List[dict], follows: List[dict] = None) -> str:
    parts = []
...
def execute_batches(db_url: str, users: List[dict], posts: List[dict], likes: List[dict], follows: List[dict] = None, batch_size: int = 500):
    conn = psycopg2.connect(db_url)
    try:
        with conn:
            with conn.cursor() as cur:
...
def main():
    ...
    posts = generate_posts(users, args.posts_per_user, start, target_user_id=args.user_id)
    likes = generate_likes(users, posts, args.likes, start, end)
    follows = generate_follows(users, args.follows, start, end, target_user_id=args.user_id) if args.follows > 0 else []
